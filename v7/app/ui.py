...
def auth_sidebar(service: AppService):
    ...
    if user:
        ...
        if status:
            ...
            st.sidebar.caption(f"今日剩余免费 AI 点评：{quota} 次（本地基础点评不限次）")
    ...
    with login_tab:
        ...
        if st.button("登录"):
            ...
            if logged_in:
                ...
                st.rerun()
            st.error("用户名或密码错误、账号已被注销，或尝试过于频繁。")
    with register_tab:
    ...
    if image_file:
        ...
        if st.button("生成观察提示"):
            ...
            if obs.get("source") == "fallback":
                st.warning(FALLBACK_MESSAGES.get(obs.get("fallback_reason"), "当前为基础提示（AI 暂不可用）。"))
...
def _admin_dashboard_tab(service: AppService):
    ...
    c1.metric("注册用户", stats["users_total"], f"近7天 +{stats['new_users_7d']}")
    c2.metric("作文提交总数", stats["submissions_total"], f"今日 {stats['submissions_today']}")
    c3.metric("AI 点评次数（免费额度）", stats["ai_calls_total"], f"今日 {stats['ai_calls_today']}")
    c4.metric("有效会员", stats["active_members"])
    c5, c6, c7, c8 = st.columns(4)
    role_names = {"student": "学生", "teacher": "老师", "parent": "家长", "admin": "管理员"}
    c5.metric("学生", stats["users_by_role"].get("student", 0))
    c6.metric("老师", stats["users_by_role"].get("teacher", 0))
    c7.metric("已注销账号", stats["disabled_users"])
    c8.metric("已核销收入（元）", stats["paid_revenue"], f"待核销订单 {stats['pending_orders']}")
    st.markdown("#### 各角色账号构成")
    dataframe([{"角色": role_names.get(k, k), "数量": v} for k, v in sorted(stats["users_by_role"].items())])
    st.markdown("#### 近 14 天作文提交趋势")
    if stats["daily_submissions"]:
        df = pd.DataFrame(stats["daily_submissions"])
        st.bar_chart(df.set_index("day")["count"])
    else:
        st.caption("近 14 天暂无提交。")
...
def _admin_users_tab(service: AppService, user: dict):
        ...
        [
            {
                ...
                "年级": u["grade"],
                "班级": u["class_name"],
                "状态": "正常" if u.get("status", "active") == "active" else "已注销",
                "注册时间": u["created_at"][:10],
            }
            for u in users
        ]
    )
    ...
    st.markdown("#### 注销 / 重新开通账号")
    if users:
        target = st.selectbox(
            "选择账号",
            [u["username"] for u in users],
            format_func=lambda name: next(
                f"{u['username']}（{u['real_name']}，{u['role']}，"
                f"{'正常' if u.get('status', 'active') == 'active' else '已注销'}）"
                for u in users
                if u["username"] == name
            ),
        ...
        with col_off:
            if st.button("注销该账号（停用登录，数据保留）"):
                ok, message = service.set_user_active(user["username"], target, active=False)
                (st.success if ok else st.error)(message)
        with col_on:
            if st.button("重新开通该账号"):
                ok, message = service.set_user_active(user["username"], target, active=True)
                (st.success if ok else st.error)(message)

        st.markdown("#### 重置密码")
        new_pwd = st.text_input("新密码", type="password", key="admin_reset_pwd")
        if st.button("重置所选账号密码") and new_pwd:
...
def _admin_ai_tab(service: AppService):
    ...
    if status["configured"]:
        st.success(f"已配置 AI 服务：模型 `{status['model']}`，接口 `{status['base_url']}`")
    else:
        st.error(
            "未配置 OPENAI_API_KEY —— 学生只能收到本地基础点评，不会调用 AI。"
            "请在环境变量或 .streamlit/secrets.toml 中设置 OPENAI_API_KEY 后重启应用。"
        )
    if status["last_success_at"]:
        st.caption(f"最近一次 AI 调用成功：{status['last_success_at']}")
    if status["last_error"]:
        st.warning(f"最近一次 AI 调用失败：{status['last_error']}")
    if st.button("测试 AI 连接"):
        with st.spinner("正在调用 AI ..."):
...
FALLBACK_MESSAGES = {
    FALLBACK_QUOTA: "今日免费 AI 点评已用完，本次为本地基础点评（不限次）。升级会员可享不限次 AI 智能点评。",
    FALLBACK_NO_API_KEY: "系统尚未配置 AI 服务（OPENAI_API_KEY），本次为本地基础点评。请联系管理员开启 AI 点评。",
    FALLBACK_RATE_LIMITED: "请求过于频繁已被限流，本次为本地基础点评。稍等一分钟再试即可使用 AI 点评。",
    FALLBACK_API_ERROR: "AI 服务暂时不可用，本次为本地基础点评。稍后重试，或联系管理员查看「AI 状态」。",
}
...
def show_feedback(title: str, genre: str, essay: str, feedback: dict, subject: str = SUBJECT_CHINESE):
    ...
    if feedback.get("source") == "llm":
        st.caption("✅ 本次点评由 AI 智能生成。")
    elif feedback.get("source") == "fallback":
        reason = feedback.get("fallback_reason")
        st.warning(FALLBACK_MESSAGES.get(reason, "本次为本地基础点评（AI 暂不可用）。"))
    st.markdown("### 教师点评版")
