...
class SQLiteStore:
    ...
    def initialize(self) -> None:
        with self.connect() as conn:
            ...
            self._migrate_user_status(conn)
        self.seed_demo_users_if_enabled()

    @staticmethod
    def _migrate_user_status(conn: sqlite3.Connection) -> None:
        """v7: users.status ('active' / 'disabled') for manual开通/注销 by admins."""
    ...
    def set_user_status(self, username: str, status: str) -> bool:
        if status not in ("active", "disabled"):
            return False
        with self.connect() as conn:
            cur = conn.execute("UPDATE users SET status=? WHERE username=?", (status, username))
    ...
    def list_users(self, limit: int, offset: int = 0) -> list[dict[str, Any]]:
        with self.connect() as conn:
            rows = conn.execute(
                """
                SELECT username, real_name, role, grade, class_name, status, created_at
                FROM users
                ORDER BY created_at DESC
                LIMIT ? OFFSET ?
                """,
    ...
    def dashboard_stats(self) -> dict[str, Any]:
        """Aggregate operating metrics for the admin overview page."""
        today = today_str()
        week_ago = (date.today() - timedelta(days=6)).isoformat()
        with self.connect() as conn:
            users_by_role = {
                row["role"]: row["c"]
                for row in conn.execute("SELECT role, COUNT(*) AS c FROM users GROUP BY role").fetchall()
            }
            disabled_users = conn.execute(
                "SELECT COUNT(*) AS c FROM users WHERE status='disabled'"
            ).fetchone()["c"]
            new_users_7d = conn.execute(
                "SELECT COUNT(*) AS c FROM users WHERE created_at >= ?", (week_ago,)
            ).fetchone()["c"]
            submissions_total = conn.execute("SELECT COUNT(*) AS c FROM submissions").fetchone()["c"]
            submissions_today = conn.execute(
                "SELECT COUNT(*) AS c FROM submissions WHERE created_at >= ?", (today,)
            ).fetchone()["c"]
            submissions_7d = conn.execute(
                "SELECT COUNT(*) AS c FROM submissions WHERE created_at >= ?", (week_ago,)
            ).fetchone()["c"]
            daily_submissions = rows_to_dicts(
                conn.execute(
                    """
            ...
            ai_calls_today = conn.execute(
                "SELECT COALESCE(SUM(count), 0) AS c FROM ai_usage WHERE usage_date=?", (today,)
            ).fetchone()["c"]
            ai_calls_total = conn.execute("SELECT COALESCE(SUM(count), 0) AS c FROM ai_usage").fetchone()["c"]
            active_members = conn.execute(
                """
                SELECT COUNT(DISTINCT username) AS c FROM subscriptions
            ...
            paid_row = conn.execute(
                "SELECT COUNT(*) AS c, COALESCE(SUM(amount), 0) AS revenue FROM orders WHERE status='paid'"
            ).fetchone()
        return {
            ...
            "paid_orders": paid_row["c"],
            "paid_revenue": paid_row["revenue"],
        }
