...
logger = logging.getLogger("essay_app.llm")

# Machine-readable reasons attached to fallback feedback so the UI can tell
# the user exactly why the local engine answered instead of the AI.
FALLBACK_NO_API_KEY = "no_api_key"
FALLBACK_RATE_LIMITED = "rate_limited"
FALLBACK_API_ERROR = "api_error"
FALLBACK_QUOTA = "quota_exhausted"
...
class LLMService:
    ...
    def status(self) -> Dict[str, Any]:
        """Runtime diagnostics for the admin dashboard."""
        return {
    ...
    def test_connection(self) -> tuple[bool, str]:
        """One tiny live call so an admin can verify the AI path end to end."""
        if not self.is_configured():
            return False, "未配置 OPENAI_API_KEY，无法调用 AI。"
        try:
            ...
            return True, f"AI 连接正常（模型 {self.settings.openai_model}）：{resp.choices[0].message.content!r}"
    ...
    def _record_error(self, exc: Exception) -> None:
        ...
        logger.warning("LLM call failed, using local fallback: %s", self.last_error)
    ...
    def essay_feedback(
        ...
        if not self.settings.openai_api_key:
            fallback["fallback_reason"] = FALLBACK_NO_API_KEY
            return fallback
        if not self.limiter.allow(user_key):
            fallback["fallback_reason"] = FALLBACK_RATE_LIMITED
            return fallback
        try:
            from openai import OpenAI  # type: ignore
            ...
            self._record_error(exc)
            fallback["fallback_reason"] = FALLBACK_API_ERROR
            return fallback

    def ocr_essay_image(self, image: Image.Image, user_key: str = "anonymous") -> Dict[str, Any]:
        """v8：识别手写作文照片中的文字（依赖视觉大模型，无本地回退）。

        返回 {"ok": True, "text": str, "source": "llm"} 或
        {"ok": False, "text": "", "reason": <fallback 常量>, "error": 可选}。
        """
        if not self.is_configured():
            return {"ok": False, "text": "", "reason": FALLBACK_NO_API_KEY}
        if not self.limiter.allow(f"ocr:{user_key}"):
            return {"ok": False, "text": "", "reason": FALLBACK_RATE_LIMITED}
        try:
            from openai import OpenAI  # type: ignore
            ...
            prompt = textwrap.dedent(
                """
                这是一张学生手写作文的照片。请逐字转写图片中的作文内容：
                - 保留原有段落划分（段落之间用换行分隔）；
                - 不要纠正学生的错别字、语法或拼写错误，原样转写；
                - 忽略与作文正文无关的内容（如页眉、稿纸格线、订正符号）；
                - 如果完全无法辨认，text 返回空字符串。
                输出 JSON：{"text": 转写出的作文全文}
                """
            ).strip()
            resp = client.chat.completions.create(
                model=self.settings.openai_model,
                ...
                response_format={"type": "json_object"},
                messages=[
                    {"role": "system", "content": "你是认真细致的手写文字转写助手，只输出合法JSON。"},
                    {
                        "role": "user",
                        "content": [
            ...
            self._record_error(exc)
            return {"ok": False, "text": "", "reason": FALLBACK_API_ERROR, "error": str(exc)}
    ...
    def image_prompts(self, image: Image.Image, grade: str, user_key: str = "anonymous") -> Dict[str, Any]:
        if not self.settings.openai_api_key:
            ...
            prompts["fallback_reason"] = FALLBACK_NO_API_KEY
            return prompts
        if not self.limiter.allow(f"image:{user_key}"):
            prompts = fallback_image_prompts(grade)
            prompts["fallback_reason"] = FALLBACK_RATE_LIMITED
            return prompts
        try:
            from openai import OpenAI  # type: ignore
            ...
            self._record_error(exc)
            prompts = fallback_image_prompts(grade)
            prompts["fallback_reason"] = FALLBACK_API_ERROR
            return prompts
