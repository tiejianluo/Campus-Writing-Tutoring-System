...
class BilingualUIAcceptance(unittest.TestCase):
    """v8 验收 1：系统必须提供中文和英文两种 UI。"""

    def test_two_ui_languages_available(self):
        from app.i18n import LANG_EN, LANG_ZH, LANGS

        self.assertIn(LANG_ZH, LANGS)
        self.assertIn(LANG_EN, LANGS)

    def test_core_flows_translated_in_both_languages(self):
        """核心界面（登录/注册、学生菜单、点评、会员、管理后台）中英文都有且不同。"""
        from app.i18n import LANG_EN, LANG_ZH, tr

        core_keys = (
            "tab_login", "tab_register", "login_btn", "register_btn",
            "menu_write", "menu_handwriting", "menu_picture", "menu_rewrite",
            "menu_versions", "menu_growth", "menu_membership",
            "feedback_title", "teacher_version", "student_version",
            "member_center", "admin_home", "tab_dashboard", "tab_accounts",
        )
        for key in core_keys:
            zh = tr(key, LANG_ZH)
            en = tr(key, LANG_EN)
            self.assertNotEqual(zh, key, f"{key} 缺中文")
            self.assertNotEqual(en, key, f"{key} 缺英文")
            self.assertNotEqual(zh, en, f"{key} 中英文相同，疑似未翻译")

    def test_default_language_is_chinese(self):
        from app.i18n import DEFAULT_LANG, LANG_ZH

        self.assertEqual(DEFAULT_LANG, LANG_ZH)


class HandwritingEssayAcceptance(unittest.TestCase):
    """v8 验收 2：学生可上传手写作文照片 → 提取文字 → 点评并修改。"""

    HANDWRITTEN_TEXT = "今天我和妈妈去公园。首先我们喂了鸽子，然后放了风筝，最后开心地回家了。"
    ...
    def _stub_ocr(self, result):
        self.service.llm.ocr_essay_image = lambda image, user_key="anonymous": dict(result)

    def test_full_flow_extract_review_save_and_revise(self):
        """上传→识别→点评保存→继续改写的完整链路。"""
        from PIL import Image

        self.service.llm.is_configured = lambda: True
        self._stub_ocr({"ok": True, "text": self.HANDWRITTEN_TEXT, "source": "llm"})
        image = Image.new("RGB", (80, 60), "white")

        extracted = self.service.ocr_extract_essay("stu1", image)
        self.assertTrue(extracted["ok"])
        self.assertEqual(extracted["text"], self.HANDWRITTEN_TEXT)

        # 学生核对后（可修改）提交点评并保存
        edited = extracted["text"] + "这一天真难忘！"
        sid, feedback = self.service.review_and_save_submission(
            "stu1", "三年级", "写事", "游公园", "游公园", edited
        )
        self.assertGreater(sid, 0)
        self.assertIn("teacher_feedback", feedback)
        growth = self.service.store.list_growth_records("stu1")
        self.assertEqual(growth[0]["topic"], "游公园")

        # 识别出的作文同样支持"点评和修改"（改写为新版本）
        v2, _ = self.service.rewrite_submission("stu1", sid, edited + "我下次还想去。")
        self.assertEqual(v2, 2)

    def test_ocr_unavailable_gives_reason_and_typing_still_works(self):
        """未配置 AI 时识别给出明确原因，且学生仍可手动输入完成点评。"""
        result = self.service.ocr_extract_essay("stu1", object())
        self.assertFalse(result["ok"])
        self.assertEqual(result["reason"], "no_api_key")
        sid, _ = self.service.review_and_save_submission(
            "stu1", "三年级", "写事", "题", "题", self.HANDWRITTEN_TEXT
        )
        self.assertGreater(sid, 0)

    def test_ocr_shares_daily_ai_quota(self):
        """识别与 AI 点评共用每日免费额度；额度用完时给出 quota 原因。"""
        self.service.llm.is_configured = lambda: True
        self._stub_ocr({"ok": True, "text": self.HANDWRITTEN_TEXT, "source": "llm"})
        quota = self.service.settings.free_ai_daily_quota
        for _ in range(quota):
            self.assertTrue(self.service.ocr_extract_essay("stu1", object())["ok"])
        self.assertEqual(self.service.ai_quota_left("stu1"), 0)
        blocked = self.service.ocr_extract_essay("stu1", object())
