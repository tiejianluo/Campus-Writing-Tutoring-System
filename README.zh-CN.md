...
系统的 **教育价值** 在支持写作教学，而不是替代教师。它最适合作为形成性辅导工具，为学生提供即时反馈，同时帮助教师和家长了解仍需指导的地方。
...
![Status](https://img.shields.io/badge/Status-v8%20%E5%8F%8C%E8%AF%AD%E7%95%8C%E9%9D%A2%2B%E6%89%8B%E5%86%99%E8%AF%86%E5%88%AB%E7%89%88-16A34A?style=for-the-badge)
...
- **v8**：**双语界面 + 手写作文识别版**。新增**中文/英文双语 UI**——侧边栏一键切换，全部界面（学生/老师/家长/管理员/点评/会员中心）即时换语言，240+ 条文案集中在 `app/i18n.py` 双语目录中管理（教学内容保持本来语言）；新增**手写作文上传**——学生拍照上传手写作文，视觉大模型逐字转写（保留段落、不改错别字），学生核对修正后走标准的点评 → 保存 → 改写 → 版本对比流程。识别与 AI 点评**共用每日免费额度**（会员不限次，识别失败不扣额度），识别不可用时给出明确原因且始终可手动输入。测试增至 98 个（v7 的 70 个全部保留 + 新增 28 个）。完整使用说明见 [`v8/README.md`](v8/README.md)。
...
- **双语界面（v8）**：侧边栏一键切换中文/英文界面；作文题库、模板等教学内容保持本来语言。
- **手写作文上传（v8）**：拍照上传手写作文，AI 视觉模型逐字识别（保留段落、不改错别字），核对修正后即可点评保存，并进入改写/对比/成长档案的同一套流程。
...
v8 在 v7 全部功能之上新增：中文/英文双语界面（即时切换）、手写作文照片上传与视觉大模型文字识别（识别结果进入标准点评-改写流程）。
...
| **v8** | **双语界面（中/英）+ 手写作文识别点评** | `v8/campus_essay_system.py` | **98** |
...
分角色（学生/老师/家长/管理员）的完整**使用说明**见 [`v8/README.md`](v8/README.md) 与 [`v7/README.md`](v7/README.md)。
...
3. 选择 GitHub 仓库和分支。
4. 设置主文件路径：
   - v8 双语界面 + 手写识别版（推荐）：`v8/campus_essay_system.py`
   - v7 运营版：`v7/campus_essay_system.py`
...
运行 v8 测试套件（单元 + 系统 + 验收 + 双语 + 手写识别）：

```bash
cd v8
python -m unittest discover -s testcode -p 'test_*.py'   # 全部 98 个测试
python -m unittest testcode.test_unit_core               # 单元测试
python -m unittest testcode.test_system_flows            # 系统测试（含管理后台）
python -m unittest testcode.test_acceptance              # 验收测试（含双语与手写验收）
...
| v8 | 98 个测试通过（v7 全部测试 + 双语 + 手写识别 + 新验收） |
| 根目录最新版本 | 64 个测试通过 |
